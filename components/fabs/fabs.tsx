...
const Fabs = ({btns}) => {
  ...
  const AnimatedIcon = ({ btn, style, textStyle, action}) => {
    const onPressHandler = () => {
      ...
      return (
        ...
        <Animated.View className="absolute right-4 bg-blue-500 h-50 w-50 rounded-full flex flex-row items-center overflow-hidden" style={[{backgroundColor: btn?.colors?.background},style]}>
          <Pressable onPress={onPressHandler} className="flex flex-row items-center w-full h-full">
          <View className="w-12 h-12 justify-center items-center">
            {btn?.icon}
          </View>
          <Animated.Text className="text-lg" style={[{color: btn?.colors?.foreground}, textStyle]}>{btn?.text}</Animated.Text>
          </Pressable>
        </Animated.View>
  ...
  return (
    <>
      ...
      <Pressable className="fixed bottom-[100] right-[15] bg-[#06A5FF] w-[50px] h-[50px] rounded-[25px] flex justify-center items-center overflow-hidden" onPress={handlePress} style={{position: "absolute", bottom: 70, right: 15}}>
